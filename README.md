...
2. Во время записи OLED показывает QR-код страницы подписки на документы текущего собрания. Для остановки кнопку нужно удерживать не менее 1,5 секунды.
...
5. Таймер повторяет отправку на VPS; OLED показывает «ОЖИДАЕТ СЕТЬ», пока `.ready` не получил `.uploaded`.
