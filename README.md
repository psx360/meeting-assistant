...
Аппаратно-программный диктофон собраний: стереозапись с двух INMP441, управление кнопкой и OLED-платой HW-787AB, настройка Wi‑Fi по BLE, отложенная отправка при отсутствии сети, транскрибация на VPS и рассылка текста через Telegram/VK.
...
2. Запись автоматически останавливается через 6 часов; за 15 минут OLED показывает предупреждение.
3. После остановки части объединяются без повторного кодирования и помещаются в устойчивую локальную очередь. OLED показывает процент подготовки и загрузки.
4. Таймер повторяет отправку на VPS; OLED показывает «ОЖИДАЕТ СЕТЬ», пока `.ready` не получил `.uploaded`.
5. VPS режет длинную запись на части по 20 минут и транскрибирует их моделью `gpt-4o-transcribe` с фиксированным языком `ru`.
6. Полученный текст без диаризации, исправлений и пересказа рассылается подписчикам Telegram и VK.
